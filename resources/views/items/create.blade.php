...
<div class="mx-auto w-full max-w-6xl px-4 py-6">
    ...
    <form method="POST" action="{{ route('items.store') }}" class="space-y-6"
        x-data="serviceForm({
            ...
            salePrice: {{ (float) old('sale_price', 0) }},
            costPrice: {{ (float) old('cost_price', 0) }},
            commissionPercent: {{ (float) old('cost_structure_commission_percent', 0) }},
            rows: @js(old('cost_structure', [])),
        ...
        <div id="service-config" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="text-sm font-semibold text-slate-700">Tiempo estimado del servicio (minutos)</label>
                    <input type="number" name="estimated_duration_minutes" :disabled="type !== 'service'" value="{{ old('estimated_duration_minutes') }}" min="1" class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Roles autorizados</label>
                    <select name="authorized_roles[]" multiple :disabled="type !== 'service'" class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                        @foreach (['Administrador', 'Veterinario', 'Auxiliar', 'Groomer', 'Bañador'] as $role)
                            <option value="{{ $role }}" @selected(in_array($role, old('authorized_roles', []), true))>{{ $role }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                    ...
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-50 text-slate-700">
                            <tr>
                                ...
                                <th class="p-2 text-left">Costo producto</th>
                                <th class="p-2 text-left">Cantidad</th>
                                <th class="p-2 text-left">Costo x ml</th>
                                <th class="p-2 text-left">Mls proceso</th>
                                <th class="p-2 text-left">Costo aplicación</th>
                                <th class="p-2"></th>
                            </tr>
                        </thead>
                        <tbody>
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="border-t border-slate-100">
                                    <td class="p-2">
                                        <select :name="`cost_structure[${index}][item_id]`" x-model.number="row.item_id" @change="applyProductData(row)" :disabled="type !== 'service'" class="w-full rounded border-slate-200 text-sm">
            ...
            <div class="grid gap-3 text-sm md:grid-cols-2">
                <div>
                    <label class="font-semibold text-slate-700">Comisión colaborador (%)</label>
                    <input type="number" name="cost_structure_commission_percent" :disabled="type !== 'service'" x-model.number="commissionPercent" min="0" max="100" step="0.01" class="mt-1 w-full rounded-lg border-slate-200 px-3 py-2">
                </div>
...
<div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-4 space-y-4">
                <div class="flex items-center gap-4">
                    ...
                    <label class="flex items-center gap-2 text-sm font-semibold text-slate-700">
                        <input type="checkbox" name="inventariable" value="1" x-model="inventariable" :disabled="type === 'service'" class="rounded border-slate-300 text-mint-600"> Inventariable
                    </label>
                    <input type="hidden" name="track_inventory" value="0">
                    <label class="flex items-center gap-2 text-sm font-semibold text-slate-700">
                        <input type="checkbox" name="track_inventory" value="1" x-model="trackInventory" :disabled="type === 'service'" class="rounded border-slate-300 text-mint-600"> Controlar stock
                    </label>
                </div>
                <div x-show="inventariable || trackInventory" x-cloak class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="stock" step="0.001" min="0" value="{{ old('stock') }}" class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Stock mínimo (alerta)</label>
                        <input type="number" name="cantidad" step="0.001" min="0" value="{{ old('cantidad') }}" class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700">Guardar</button>
        </div>
    </form>
</div>
