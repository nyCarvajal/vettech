        ...
        <div x-show="type === 'service'" x-cloak class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="estimated_duration_minutes" value="{{ old('estimated_duration_minutes') }}" min="1" class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Roles autorizados</label>
                    <select name="authorized_roles[]" multiple class="mt-2 w-full rounded-lg border-slate-200 px-3 py-2 text-sm">
                        ...
                        <tbody>
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="border-t border-slate-100">
                                    <td class="p-2">
                                        <select :name="`cost_structure[${index}][item_id]`" x-model.number="row.item_id" @change="applyProductData(row)" class="w-full rounded border-slate-200 text-sm">
            ...
            <div class="grid gap-3 text-sm md:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="cost_structure_commission_percent" x-model.number="commissionPercent" min="0" max="100" step="0.01" class="mt-1 w-full rounded-lg border-slate-200 px-3 py-2">
                </div>
