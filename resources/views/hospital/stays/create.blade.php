        ...
        <form method="post" action="{{ route('hospital.stays.store') }}" class="space-y-6">
            ...
            @else
                @if(old('patient_id') || request('patient_id'))
                    <x-alert type="error">No encontramos un paciente con el ID proporcionado. Verifica el número o selecciona otro.</x-alert>
                @endif
                <div class="space-y-2">
                        ...
                        @else
                            ...
                            @foreach($cages as $cage)
                                <option value="{{ $cage->id }}" {{ old('cage_id') == $cage->id ? 'selected' : '' }}>
                                    {{ $cage->name }}{{ $cage->location ? ' (' . $cage->location . ')' : '' }}
                                </option>
                            @endforeach
                        @endif
                    </select>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <select name="severity" class="border border-purple-200 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 rounded-lg p-3 w-full">
                        <option value="stable" {{ old('severity') === 'stable' ? 'selected' : '' }}>Estable</option>
                        <option value="observation" {{ old('severity') === 'observation' ? 'selected' : '' }}>Observación</option>
                        <option value="critical" {{ old('severity') === 'critical' ? 'selected' : '' }}>Crítico</option>
                    </select>
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700">Diagnóstico</label>
                    <textarea name="diagnosis" class="border border-purple-200 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 rounded-lg p-3 w-full" rows="2" placeholder="Diagnóstico inicial">{{ old('diagnosis') }}</textarea>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <textarea name="plan" class="border border-purple-200 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 rounded-lg p-3 w-full" rows="3" placeholder="Tratamientos o cuidados sugeridos">{{ old('plan') }}</textarea>
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-gray-700">Dieta</label>
                    <textarea name="diet" class="border border-purple-200 focus:ring-2 focus:ring-purple-400 focus:border-purple-400 rounded-lg p-3 w-full" rows="3" placeholder="Alimentación recomendada">{{ old('diet') }}</textarea>
                </div>
            </div>
